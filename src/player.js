...
export class Player { // Wir bauen eine Vorlage für unseren Spieler.
  constructor(spriteImage, spawnX, spawnY) { // Hier starten wir den Spieler mit Bild und Start-Ort.
    ...
    this.animations = { // Hier legen wir fest, welche Bilder zu welcher Bewegung gehören.
      ...
      jump: [ // Diese Bilder zeigen den Sprung.
        { col: 0, row: 3 }, // Sprungbild 1.
        { col: 1, row: 3 }, // Sprungbild 2.
        { col: 2, row: 3 }, // Sprungbild 3.
        { col: 3, row: 3 }, // Sprungbild 4.
        { col: 4, row: 3 }, // Sprungbild 5.
        { col: 5, row: 3 }, // Sprungbild 6.
      ], // Ende der Sprungbilder.
      fall: [{ col: 5, row: 3 }], // Dieses eine Bild nutzen wir fürs Fallen.
      duck: [{ col: 1, row: 2 }], // Dieses eine Bild nutzen wir fürs Ducken.
  ...
  resolveCollisionsY(level) { // Diese Funktion stoppt den Spieler bei Boden/Decke.
    ...
    for (let row = top; row <= bottom; row++) { // Wir prüfen jede berührte Reihe.
      for (let col = left; col <= right; col++) { // Wir prüfen jede berührte Spalte.
        ...
        if (this.vy > 0) { // Wenn wir nach unten fallen und Boden treffen...
          this.y = row * tileSize - this.height; // ...setzen wir den Spieler direkt auf den Boden.
          this.vy = 0; // Vertikale Bewegung stoppen.
          this.onGround = true; // Jetzt stehen wir auf dem Boden.
        } else if (this.vy < 0) { // Wenn wir nach oben springen und Decke treffen...
          this.y = (row + 1) * tileSize; // ...setzen wir den Spieler direkt unter die Decke.
          this.vy = 0; // Vertikale Bewegung stoppen.
        } // Ende Y-Richtungskorrektur.
      } // Ende Spalten-Schleife.
    } // Ende Reihen-Schleife.
  } // Ende von resolveCollisionsY.
