...
// Verwaltet komplette Spieler-Runtime: Input, Physik, Treffer, Respawn und Animation.
export class Player {
  constructor(spriteImage, spawnX, spawnY) {
    ...
    this.vx = 0;
    this.vy = 0;
    this.moveSpeed = 250; // Horizontale Laufgeschwindigkeit in Pixel/Sekunde.
    this.jumpForce = 620; // Negativer Y-Impuls beim Absprung.
    this.gravity = 1800; // Beschleunigung nach unten.
    this.onGround = false;
    this.facing = 1;

    this.sprite = new SpriteSheet(spriteImage, 33, 32);
    this.spriteScale = 3;
    this.drawOffsetX = -37; // Kollisionsbox und Sprite sind bewusst versetzt.
    this.drawOffsetY = 0;
    this.animations = {
      idle: [
        { col: 0, row: 0 },
  ...
  startRespawn() {
    ...
    this.currentFramePointer = 0;
    this.animationTimer = 0;
    this.worldResetRequested = true; // Signal an Game, Gegner/Pickups/Schalter zurueckzusetzen.
  }
  ...
  takeHit(hitFromX) {
    ...
    this.vx = hitFromX < this.x ? GAMEPLAY.knockbackX : -GAMEPLAY.knockbackX; // Knockback weg von der Trefferquelle.
  ...
  readJumpInput(input) {
    return (
      // Space, Pfeil hoch oder W loesen denselben Jump-Trigger aus.
  ...
  moveVertically(level, dt) {
    ...
    this.y += this.vy * dt;
    this.onGround = false;
    this.resolveCollisionsY(level, previousBottom);
  ...
  resolveCollisionsY(level, previousBottom) {
    const tileSize = level.tileDisplaySize;
    const left = Math.floor(this.x / tileSize);
    const right = Math.floor((this.x + this.width - 1) / tileSize);
    if (this.vy > 0) {
      ...
      for (let col = left; col <= right; col++) {
        const tileTop = bottom * tileSize;
        const hitsSolid = level.isSolidTile(col, bottom);
        const hitsOneWay =
          level.isOneWayTile(col, bottom) && previousBottom <= tileTop + 2; // One-way nur von oben blockierend.
        if (!hitsSolid && !hitsOneWay) continue;
  ...
  updateAnimation(dt, horizontalDirection, isDucking) {
    if (this.playDuckAnimation(dt, isDucking)) return; // Duck hat Vorrang vor Lauf/Idle.
  ...
  selectMovementAnimation(horizontalDirection) {
    if (!this.onGround) return this.vy < 0 ? "jump" : "fall"; // In der Luft nur jump/fall.
  ...
  draw(ctx, camera) {
    if (this.hitInvulnerabilityTimer > 0) {
      const flashVisible =
        Math.floor(this.hitInvulnerabilityTimer * 12) % 2 === 0;
