...
import { CANVAS_HEIGHT, CANVAS_WIDTH, ASSET_PATHS, COLLECTIBLE_TYPE } from "./constants.js";
...
export class Game {
    ...
    async start() {
        ...
        this.player = new Player(playerSprite, this.level.spawnX, this.level.spawnY); // Spawn kommt direkt aus der Level-Definition.
        this.enemyAtlasImage = enemyAtlas;
        this.pickupSprite = new SpriteSheet(pickupAtlas, HUD_SPRITE.frameWidth, HUD_SPRITE.frameHeight);
    ...
    drawHudIcon(frame, x, y, alpha = 1) {
        ...
        const source = frame.sx !== undefined ? frame : this.hudSprite.frameAt(frame.col, frame.row);
    ...
    drawDiamondHud(startX, startY) {
        ...
        this.ctx.fillText(String(this.score), startX + iconSize + HUD_LAYOUT.scoreGap, startY + iconSize - 7); // Score rechts neben dem Diamant-Icon.
    }
    ...
    updateSwitch() {
        const wantsInteract = this.input.wasPressed("KeyE") || this.input.wasPressed("ArrowUp"); // Interaktions-Trigger fuer den Level-Schalter.
    ...
    onCollect(collected) {
        if (collected.type === COLLECTIBLE_TYPE.diamond) this.score += collected.value; // value ist in constants als diamondScore definiert.
        ...
        if (collected.type === COLLECTIBLE_TYPE.cherry) {
            ...
            this.player.hearts = Math.min(this.player.maxHearts, this.player.hearts + 1); // Sonst nur normale Heilung um +1.
        }
    }
